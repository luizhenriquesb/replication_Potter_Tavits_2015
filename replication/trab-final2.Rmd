...
No código disponibilizado para replicação, somente temos a informação de como os outliers foram excluídos, mas não sob quais critérios. Trata-se de um comando para filtrar os países cujo número efetivo de partidos após as eleições fosse menor que 9.2.
...
Outra forma de analisar os outliers é através de uma análise gráfica dos resíduos pelas observações influentes. Aqui, é necessário primeiro rodar o modelo para depois plotar o gráfico. Também cabe a ressalva de que o gráfico abaixo nos ajuda a identificar valores influentes (que afetam a linha de regressão), que não necessarimente são outliers.
...
No gráfico acima, valores considerados influentes deveriam aparecer depois da linha tracejada (o que significaria que eles possuem altos nível de "distanância de Cook", método estatístico para identificar esses valores influentes). Como é possível perceber, não existem valores influentes.
...
Na análise dos resíduos padronizados que fizemos, foram considerados valores outliers aqueles cujos resíduos padronizados fossem maior que 2 ou menor que -2. Assim, os países considerados outliers através desses critérios foram `r paste(paises_outliers, sep = ", ")`.

### Modelos apresentados no artigo

Como foi dito acima, os dois modelos que os autores utilizaram na análise foram gerados a partir de uma base sem aqueles três países considerados "outliers", os quais, embora identificados por meio das análises de outliers que fizemos, eles foram identificados sob métodos diferentes. A despeito disso, abaixo apresentamos os dois modelos apresentados pelos autores no paper, excluindo a Libéria, a Albânia e o Brasil (assim os autores fizeram).
...
Os resultados da regressão estão de acordo com os resultados apresentados no artigo. A única diferença reside no fato de que os coeficientes apresentados nos artigos estão arredondados em duas casas decimais, enquanto aqui estão em três casas. Mas são resultados essencialmente iguais.

### Modelos mencionados no artigo

Os autores mencionam em notas de rodapé outras análises que fizeram. Vamos refazer essas análises aqui.

### 1. Contribuição dos componentes da índice de *fund parity* (nota 42)

Na 42º nota de rodapé os autores buscam verificar se alguma das quatros variáveis que compõem o fund parity trabalha contra as outras três.

#### 1.1 Algum componente trabalha contra os outros?

Os autores argumentam que todos os componentes estão positivamente associadas à paridade de fundos, embora somente a limitação da quantidade de doação permitida seja estatisticamente significativa. De fato é isso o que acontece.
...
#### 1.2 E quando cada componente é analisado individualmente?
...
2. Estimam uma reta de regressão entre o NEP e o fund parity.
...
OBS: No código fornecido para replicação, a análise é feita usando campaigns, mas o gráfico apresentado no artigo usa a base raw_data.
